...
var Helpers = {
	...
	charAt: function(s, i) {
		var c = s.charAt(i);
		var n = s.charAt(i + 1);
		if (Helpers.isSurrogate(c, n)) {
			return c + n;
		}
		return c;
	},
	escapeChar: function(s) {
		if (s === "<") {
			return "&lt;";
		}
		if (s === ">") {
	...
	getCellCoords: function(td) {
		var $td = $(td);
		var charX = Helpers._getNodeIndex($td);
		var charY = Helpers._getNodeIndex($td.parents("tr"));
		var $tile = $td.parents(".tilecont");
		var tileYX = $tile.attr(TILE_YX_ATTR);
		var tileCoords = stringToCoords(tileYX);
		return {
	...
	length: function(s) {
		return Helpers.getChars(s).length;
	},
	vectorLen: function() {
		var args = [];
		for (var _i = 0; _i < arguments.length; _i++) {
