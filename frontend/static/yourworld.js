...
YourWorld.World = function() {
	...
	var initChangeColor = false;
	var changeColor = function() {
		var d;
		if (_ui.colorInputModal) {
			d = _ui.colorInputModal;
			$("[name=url_input]")[0].value = ("000000" + YourWorld.Color.toString(16)).slice(-6);
		} else {
			d = document.createElement('div');
			var html = [];
			html.push('<form method="get" action="#" id="color_input_form">');
			html.push('<div id="url_input_title" style="max-width:20em"></div><br>');
			html.push('<label for="color_input">Color Code: #</label><input type="text" name="url_input" value="' + ("000000" + YourWorld.Color.toString(16)).slice(-6) + '">');
			html.push('<div id="color_input_submit"><input type="submit" value="   Go   "> or <span id="color_input_cancel" class="simplemodal-close simplemodal-closelink">cancel</span></div>');
			html.push('</form>');
			d.innerHTML = html.join('');
			$(d).hide();
			$('body').append(d);
			_ui.colorInputModal = d;
			$('#color_input_form').submit(function() {
					var colorCode = document.getElementById('color_input_form').url_input.value;
					if(!colorCode) colorCode = 0;
					$('#color_input_cancel').trigger('click'); // Close it like this.
					YourWorld.Color = parseInt(colorCode, 16)
					return false; // cancel submit
				});
		}
		$(d).modal({
